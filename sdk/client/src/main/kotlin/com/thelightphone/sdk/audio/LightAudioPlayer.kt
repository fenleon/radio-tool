...
class LightAudioPlayer internal constructor(
    ...
    private val player = createPlayer(context, usage)
    ...
    private fun setupMediaSession(context: Context) {
        try {
            ...
            LightMediaService.setActiveSession(session)
            ...
            android.util.Log.w("LightAudioPlayer", "MediaSession not available", e)
        }
    }

    private fun notifyServerOfSession(context: Context, active: Boolean) {
        scope.launch {
            callRemoteServiceMethod(
                LightServiceMethod.RegisterMediaSession,
                LightServiceMethod.RegisterMediaSession.Request(context.packageName, active)
            )
        }
    }

    @androidx.annotation.OptIn(markerClass = [UnstableApi::class])
    private fun createPlayer(context: Context, usage: LightAudioUsage): ExoPlayer {
    ...
    fun release() {
        ...
        stopPositionUpdates()
        abandonFocus()
        notifyServerOfSession(appContext, false)
        session?.release()
        session = null
        LightMediaService.setActiveSession(null)
        player.release()
        scope.cancel()
    }
