...
sealed interface LightServiceMethod<TRequest, TResponse> {
    ...
    object OpenBluetoothSettings : LightServiceMethod<Unit, Unit> {
        override val id = "OpenBluetoothSettings"
        override val requestSerializer = serializer<Unit>()
        override val responseSerializer = serializer<Unit>()
    }

    object RegisterMediaSession : LightServiceMethod<RegisterMediaSession.Request, Unit> {
        override val id = "RegisterMediaSession"
        override val requestSerializer = serializer<Request>()
        override val responseSerializer = serializer<Unit>()

        @Serializable
        data class Request(val packageName: String, val active: Boolean)
    }
}
...
val allMethods: Map<String, LightServiceMethod<*, *>> = listOf(
    ...
    LightServiceMethod.OpenBluetoothSettings,
