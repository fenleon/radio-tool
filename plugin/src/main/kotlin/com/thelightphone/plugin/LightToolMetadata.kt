    ...
    companion object {
        ...
        fun parse(file: File): LightToolMetadata {
            ...
            return LightToolMetadata(
                ...
                sharedUserId = validateSharedUserId(tool.tomlString("sharedUserId")),
                orientation = validateOrientation(tool.tomlString("orientation")),
                enableBackgroundAudio = tool.tomlBoolean("enableBackgroundAudio") ?: false,
            )
        }

        private fun validateSharedUserId(value: String?): String? {
            if (value == null) return null
            require(LightToolPolicy.TOOL_ID_PATTERN.matches(value)) {
                "tool.sharedUserId must be a lowercase dotted Java package identifier " +
                        "(e.g. com.example.shared); got '$value'"
            }
            return value
        }
