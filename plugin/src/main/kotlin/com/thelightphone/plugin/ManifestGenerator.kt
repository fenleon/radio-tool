...
object ManifestGenerator {
    fun render(metadata: LightToolMetadata): String = buildString {
        ...
        val sharedUserIdAttr = metadata.sharedUserId?.let {
            """ android:sharedUserId="${xmlAttr(it)}""""
        }.orEmpty()
        appendLine("""<manifest xmlns:android="http://schemas.android.com/apk/res/android"$sharedUserIdAttr>""")
        
        val permissions = metadata.permissions.toMutableSet()
        if (metadata.enableBackgroundAudio) {
        ...
        val backgroundService = if (metadata.enableBackgroundAudio) {
            """
            |        <service
            |            android:name="com.thelightphone.sdk.audio.LightMediaService"
